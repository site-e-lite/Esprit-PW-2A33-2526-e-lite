...
class QuizController {
    ...
    public function validateQuiz($data) {
        ...
        $seuilReussite = trim($data['seuilReussite'] ?? '');
        $niveau        = trim($data['niveau']        ?? '');
        $statut        = trim($data['statut']        ?? '');
        $idCourse      = trim($data['idCourse']      ?? '');
        ...
        // idCourse : obligatoire et doit être un entier >= 1
        if (empty($idCourse)) {
            Validator::addError('idCourse', 'Cours associé est obligatoire.');
        } elseif (!is_numeric($idCourse) || intval($idCourse) < 1) {
            Validator::addError('idCourse', 'Cours associé invalide.');
        }
    ...
    public function getQuestionsForAssignment($niveau = null) {
        ...
        // Normalise le niveau : minuscules + sans accents pour comparaison robuste
        $niveauNorm = null;
        if (!empty($niveau)) {
            $niveauNorm = $this->normalizeNiveauString($niveau);
        }
        ...
        if ($niveauNorm !== null) {
            ...
            $sql .= " AND (
                LOWER(TRIM(
                    REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(niveau,
                        'é','e'),'è','e'),'ê','e'),'É','e'),'È','e'),'Ê','e')
                )) = :niveau
                OR niveau IS NULL
                OR TRIM(niveau) = ''
            )";
            $params['niveau'] = $niveauNorm;
        }
    ...
    /**
     * Normalise un niveau : minuscules + suppression des accents courants.
     * "Débutant" → "debutant", "Intermédiaire" → "intermediaire", "Avancé" → "avance"
     */
    private function normalizeNiveauString(string $niveau): string {
    ...
    public function assignQuestionsToQuiz($idQuiz, $questionIds, $niveauQuiz = null) {
        ...
        try {
            ...
            // Détacher les questions de leur quiz précédent
            $db->prepare("UPDATE question SET idQuiz = NULL WHERE idQuestion IN ($placeholders)")
               ->execute($questionIds);

            // Attacher au nouveau quiz
    ...
    private function syncQuizQuestions($db, $idQuiz, $questionIds) {
        ...
        // Détacher toutes les questions actuellement liées à ce quiz
        $db->prepare("UPDATE question SET idQuiz = NULL WHERE idQuiz = :idQuiz")
           ->execute(['idQuiz' => $idQuiz]);
        ...
        $db->prepare("UPDATE question SET idQuiz = ? WHERE idQuestion IN ($placeholders)")
           ->execute($params);
    }
    ...
    public function validateQuizQuestionMatch($niveau, $questionIds = []) {
        ...
        try {
            $niveauNorm = $this->normalizeNiveauString($niveau);
            $ids = array_map('intval', (array) $questionIds);
            $ids = array_filter($ids, fn($id) => $id > 0);
            if (empty($ids)) return true;

            $placeholders = implode(',', array_fill(0, count($ids), '?'));

            // Normalise la colonne niveau côté SQL (accents + casse)
            $normExpr = "LOWER(TRIM(
            ...
            $sql = "SELECT
                        ...
                        SUM(CASE WHEN $normExpr = ? OR TRIM(COALESCE(niveau,'')) = '' THEN 1 ELSE 0 END) AS count_matching
                    FROM question
                    WHERE idQuestion IN ($placeholders)";

            $stmt = $db->prepare($sql);
            // Bind: niveau en position 1, puis les IDs
            $params = array_merge([$niveauNorm], array_values($ids));
            $stmt->execute($params);
            $result = $stmt->fetch();

            $total    = (int)($result['count_questions'] ?? 0);
            $matching = (int)($result['count_matching']  ?? 0);

            // Toutes les questions doivent correspondre au niveau (ou n'avoir aucun niveau défini)
            return $total > 0 && $total === $matching;
        } catch (Exception $e) {
            error_log('validateQuizQuestionMatch: ' . $e->getMessage());
