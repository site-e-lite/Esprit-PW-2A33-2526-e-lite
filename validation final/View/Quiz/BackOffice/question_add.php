...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!$questionController->validateQuestion($_POST)) {
        ...
        header('Location: ' . $__bp . '/quiz/admin/questions?added=1');
        exit;
    }
}
?>
...
<body>
    <aside class="admin-sidebar">
        ...
        <ul class="admin-nav">
            ...
            <li><a href="<?= $__bp ?>/quiz/admin/question/ajouter" class="active"><i class="fas fa-plus-square"></i> Ajouter une Question</a></li>
        </ul>
    </aside>

    <main class="admin-content">
        <a href="<?= $__bp ?>/quiz/admin/questions" class="back-link"><i class="fas fa-arrow-left"></i> Retour à la liste des questions</a>
...
<script>
// ── QCU Response Builder ──────────────────────────────────────────
(function() {
    const form       = document.getElementById('questionAddForm');
    const container  = document.getElementById('responsesContainer');
    const addBtn     = document.getElementById('addResponseBtn');

    function getRows() {
        return container ? Array.from(container.querySelectorAll('.response-row')) : [];
    }

    function updatePlaceholders() {
        getRows().forEach(function(row, i) {
            const inp = row.querySelector('input[name="reponses[]"]');
            if (inp && !inp.value) inp.placeholder = 'Réponse ' + (i + 1);
        });
    }
    ...
    function updateRemoveButtons() {
        ...
        rows.forEach(function(row) {
            const btn = row.querySelector('.remove-response-btn');
    ...
    function createRow(value) {
        ...
        inp.name = 'reponses[]';
        inp.value = value || '';
        inp.placeholder = 'Réponse ' + (getRows().length + 1);

        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'remove-response-btn';
        btn.setAttribute('aria-label', 'Supprimer cette réponse');
    ...
    function ensureMinRows(min) {
        if (!container) return;
        while (getRows().length < min) {
    ...
    // Expose pour index.js (compatibilité)
    window.ensureQuestionResponseRows = function(f) { ensureMinRows(2); };

    // ── Toggle QCU / Ouverte ──────────────────────────────────────
    window.toggleQCUHelp = function() {
        ...
        if (typeMsg) {
            typeMsg.textContent = type === 'QCU'
                ? 'Pour une QCU, ajoutez les réponses puis indiquez la bonne réponse.'
                : type === 'Ouverte'
                    ? 'Pour une question ouverte, sélectionnez Vrai / Faux.'
                    : '';
        }

        if (responsesGroup) responsesGroup.style.display = type === 'QCU'    ? 'block' : 'none';
        if (openGroup)      openGroup.style.display      = type === 'Ouverte' ? 'block' : 'none';
        if (bonneReponseText) bonneReponseText.style.display = type === 'QCU' ? 'block' : 'none';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Sync bonne réponse hidden ↔ text
        ...
        // Sync radio Vrai/Faux
        document.querySelectorAll('input[name="openChoice"]').forEach(function(r) {
            r.addEventListener('change', function() {
                if (hidden) hidden.value = this.value;
            });
            if (hidden && r.value === hidden.value) r.checked = true;
        });

        // Si des réponses ont été postées (erreur de validation), binder les boutons supprimer
        ...
        // Appliquer l'état initial selon le type sélectionné
        window.toggleQCUHelp();
    });
})();
</script>
</body>
