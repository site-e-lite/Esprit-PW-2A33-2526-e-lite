    ...
    if (isset($_POST['action']) && $_POST['action'] === 'rate_forum') {
        ...
        if ($idForum > 0 && $note >= 1 && $note <= 5) {
            try {
                ...
                // Si raterForum retourne null (erreur DB), recalculer manuellement
                if ($newAvg === null) {
                    $db2 = Config::getConnexion();
                    $s2  = $db2->prepare('SELECT ROUND(AVG(note),1) FROM forum_rating WHERE idForum = ?');
                    $s2->execute([$idForum]);
                    $newAvg = (float) $s2->fetchColumn();
                }
...
// Créer forum_rating si elle n'existe pas encore
try {
...
} catch (Exception $e) { /* table existe déjà */ }
...
<body>
    ...
    <section id="classes" class="gestion-section reveal dark-bg">
        ...
        ?>
            <div class="class-card glass-card<?= $isEnCours ? '' : '' ?>">
    ...
    <script>
        ...
        function rateForum(idForum, note) {
            ...
            fetch(FORUM_POST_URL, { method: 'POST', body: fd })
                ...
                .then(data => {
                    if (!data.success) {
                        console.warn('rateForum failed:', data.error || 'unknown');
                        return;
                    }
                    ...
                    if (wrap) {
                        wrap.querySelectorAll('.star').forEach(s => {
                            ...
                            if (sv <= note) { s.classList.add('filled');    s.style.color = '#f59e0b'; }
                            else            { s.classList.remove('filled'); s.style.color = 'rgba(255,255,255,0.15)'; }
                        });
                    }
                    // Mettre à jour le label
                    if (label) {
                        const avg = parseFloat(data.avg) || note;
                        label.textContent = avg.toFixed(1) + ' ★ (Note enregistrée)';
                    }
                })
                .catch(err => console.error('rateForum error:', err));
        }
