...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $vc = new VirtualClass(null, trim($_POST['titre']??''), trim($_POST['description']??''), trim($_POST['lienAcces']??''), trim($_POST['plateforme']??''), (int)($_POST['capacite']??30), (int)($_POST['idCourse']??0));
        $controller->addVirtualClass($vc);
        header('Location: '.$basePath.'/virtualclass?success=Classe+ajoutée'); exit;
...
<head>
    ...
    <title>Ajouter Classe Virtuelle</title>
...
<main class="main-content">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
        <h1 style="margin:0;font-size:1.8rem;"><i class="fas fa-plus-circle" style="color:var(--accent);"></i> Ajouter une Classe Virtuelle</h1>
        ...
        <form id="virtualClassForm" method="POST" class="glass-form" novalidate>
            <div class="form-grid">
                <div class="form-group full-width">
                    <label>Titre *</label>
                    <input type="text" name="titre" id="vc_titre" value="<?= htmlspecialchars($_POST['titre']??'') ?>" placeholder="Ex: Introduction au Machine Learning">
                </div>
                <div class="form-group full-width">
                    <label>Description</label>
                    <textarea name="description" id="vc_description" rows="3" placeholder="Description optionnelle…"><?= htmlspecialchars($_POST['description']??'') ?></textarea>
                </div>
                <div class="form-group full-width">
                    <label>Lien d'accès *</label>
                    <input type="text" name="lienAcces" id="vc_lienAcces" value="<?= htmlspecialchars($_POST['lienAcces']??'') ?>" placeholder="https://zoom.us/j/...">
                </div>
                <div class="form-group">
                    ...
                    <select name="plateforme" id="vc_plateforme">
                        ...
                        <?php foreach(['Zoom','Meet','Teams','Autre'] as $p): ?>
                            <option value="<?= $p ?>" <?= ($_POST['plateforme']??'')===$p?'selected':'' ?>><?= $p==='Meet'?'Google Meet':($p==='Teams'?'Microsoft Teams':$p) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label>Capacité *</label>
                    <input type="number" min="1" max="5000" name="capacite" id="vc_capacite" value="<?= htmlspecialchars($_POST['capacite']??'30') ?>">
                </div>
                <div class="form-group">
                    ...
                    <select name="idCourse" id="vc_idCourse">
                        ...
                        <?php foreach($courses as $c): ?>
                            <option value="<?= (int)$c['idCourse'] ?>" <?= (($_POST['idCourse']??'')==(string)$c['idCourse'])?'selected':'' ?>>#<?= (int)$c['idCourse'] ?> — <?= htmlspecialchars($c['titre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <button type="submit" class="btn-primary full-width" style="margin-top:1.4rem;"><i class="fas fa-save"></i> Enregistrer</button>
        </form>
    </div>
</main>
