...
if (isset($_POST['action']) && $_POST['action'] === 'scan_forum') {
    ...
    try {
        ...
        $textToScan = trim($_POST['text'] ?? '');
        if (empty($textToScan)) {
            echo json_encode(["risk" => "Low", "reason" => "Texte vide."]);
        } else {
            $evaluation = $chatbot->evaluateContentRisk($textToScan);
            echo json_encode($evaluation);
        }
    } catch (Exception $e) {
        echo json_encode(["risk" => "Low", "reason" => "Analyse IA indisponible."]);
    }
    exit;
}
...
$forumManagePath = strtok($_SERVER['REQUEST_URI'] ?? '/forum/manage', '?') ?: '/forum/manage';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['action']) && $_POST['action'] == 'add_forum') {
        ...
        header('Location: ' . $forumManagePath);
        exit;
    }
...
<body>
    ...
    <div class="modal" id="modalAddForum">
        ...
        <div class="modal-body">
            <form action="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" method="POST" class="glass-form" onsubmit="return validateForum(this)">
    ...
    <div class="modal" id="modalAddPost">
        ...
        <div class="modal-body">
            <form action="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" method="POST" class="glass-form" onsubmit="return validateAddPost(this)">
    ...
    <div class="modal" id="modalEditPost">
        ...
        <div class="modal-body">
            <form action="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" method="POST" class="glass-form" onsubmit="return validateUpdatePost(this)">
    ...
    <aside class="admin-sidebar">
        <a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/forum/manage" class="logo">e-lite<span>.</span><div style="font-size:0.8rem; letter-spacing:3px; color:var(--light-gray); font-family:var(--font-main);text-transform:uppercase;">BackOffice</div></a>
        <ul class="admin-nav">
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/admin/dashboard"><i class="fas fa-home"></i> Vue d'ensemble</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/admin/dashboard"><i class="fas fa-users"></i> Gest. Utilisateurs</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/admin/dashboard"><i class="fas fa-book-open"></i> Gest. Cours &amp; Inscr.</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/admin/dashboard"><i class="fas fa-tasks"></i> Gest. Évaluations</a></li>
            <li><a href="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" class="active"><i class="fas fa-comments"></i> Gest. Forum</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/virtualclass"><i class="fas fa-play-circle"></i> Classes Virtuelles</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/forum" style="margin-top:2rem; border: 1px dashed var(--glass-border);"><i class="fas fa-external-link-alt"></i> Voir le Site</a></li>
            <li><a href="<?= htmlspecialchars($basePath, ENT_QUOTES, 'UTF-8') ?>/logout" class="logout-btn"><i class="fas fa-sign-out-alt"></i> Déconnexion</a></li>
        </ul>
    </aside>
    ...
    <main class="admin-content">
        ...
        <header class="admin-header reveal">
            ...
            <div class="admin-profile">
                <button type="button" class="btn-primary" style="margin-right:1rem; padding: 0.6rem 1.2rem; font-size: 0.85rem; text-decoration:none; display:inline-flex; align-items:center; gap:0.4rem;" onclick="openModal('modalAddForum')"><i class="fas fa-plus"></i> Forum</button>
        ...
        <div class="glass-card reveal" style="margin-bottom: 1.5rem;">
            <form method="GET" action="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" id="filterForm">
                <div class="filter-bar">
                    ...
                    <div class="filter-actions">
                        ...
                        <a href="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" class="btn-outline" style="padding:0.55rem 1.2rem; font-size:0.85rem; text-decoration:none;">
                    ...
                    <tbody>
                        <?php if ($posts && $posts->rowCount() > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>"<?= htmlspecialchars(substr($p['contenu'], 0, 40)) ?>…"<br><span style="color:var(--accent); font-size:0.8rem;">ID Post: #<?= htmlspecialchars($p['idPost']) ?></span></td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <a href="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>?delete_post=<?= $p['idPost'] ?>" onclick="return confirm('Confirmer la suppression du message ?');" class="action-btn delete" title="Supprimer"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                        ...
                        <?php if (count($forumsData) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>?delete_forum=<?= $f['idForum'] ?>" onclick="return confirm('Confirmer la suppression du forum \\'<?= htmlspecialchars(addslashes($f['titre'])) ?>\' ?');" class="action-btn delete" title="Supprimer"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr class="no-results-row">
                                <td colspan="6">
                                    ...
                                    <?php if ($activeFilters): ?><br><a href="<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>" style="color:var(--accent); font-size:0.85rem;">Réinitialiser les filtres</a><?php endif; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <script>
        ...
        function scanForum(btn, idForum, text) {
            ...
            fetch('<?= htmlspecialchars($forumManagePath, ENT_QUOTES, 'UTF-8') ?>', { method: 'POST', body: fd })
                .then(res => {
                ...
                .catch(err => {
                    btn.classList.remove('scanning');
                    showToast("IA indisponible — contenu non analysé.", 'warning');
                });
        }
