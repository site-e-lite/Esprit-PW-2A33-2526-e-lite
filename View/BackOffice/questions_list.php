...
if (isset($_GET['delete_question'])) {
    ...
    header('Location: questions_list.php?deleted=1');
    exit;
}

$idQuizFilter = isset($_GET['idQuiz']) ? intval($_GET['idQuiz']) : null;
$questions = $questionController->afficherQuestions($idQuizFilter);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="confirm-overlay" id="confirmOverlay">
    ...
    <main class="admin-content">
        <header class="admin-header reveal">
            ...
            <div>
                <form method="GET" class="filter-form" style="display:inline-flex; gap:0.8rem; align-items:center;">
                    <label style="color: rgba(255,255,255,0.55); font-size:0.9rem;">Filtrer par Quiz ID</label>
                    <input type="number" name="idQuiz" value="<?= htmlspecialchars($idQuizFilter ?? '') ?>" placeholder="Ex: 3" class="filter-input">
                    <button type="submit" class="filter-btn"><i class="fas fa-search"></i> Filtrer</button>
                </form>
                <a href="question_add.php" class="add-btn" style="margin-left:1rem;"><i class="fas fa-plus"></i> Ajouter une Question</a>
            </div>
        </header>
                    ...
                    <?php if ($questions && $questions->rowCount() > 0): ?>
                        <?php while ($q = $questions->fetch()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($q['quizTitre'] ?? $q['idQuiz']) ?></td>
                                <td>
                                    <a href="question_update.php?id=<?= $q['idQuestion'] ?>" class="action-btn" title="Modifier"><i class="fas fa-edit"></i></a>
                                    <button class="action-btn delete" title="Supprimer" onclick="askDelete(<?= $q['idQuestion'] ?>)"><i class="fas fa-trash"></i></button>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="7" style="text-align:center; color:rgba(255,255,255,0.4); padding:2rem;">Aucune question trouvée.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
...
function askDelete(questionId) {
    document.getElementById('confirmDeleteLink').href = 'questions_list.php?delete_question=' + questionId;
