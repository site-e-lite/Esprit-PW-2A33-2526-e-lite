...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!$quizController->validateQuiz($_POST)) {
        ...
        elseif (!empty($selectedQuestionIds) && !$quizController->validateQuizQuestionMatch($niveau, $selectedQuestionIds)) {
        ...
        <div class="form-card">
            ...
            <form id="quizUpdateForm" action="" method="POST">
                ...
                <div class="field-group" id="fg-titre">
                    ...
                    <input type="text" id="titre" name="titre" value="<?= htmlspecialchars($quizData['titre']) ?>" placeholder="Titre du quiz">
                    <div class="field-msg" id="titre-msg"><?= htmlspecialchars($errors['titre'] ?? '') ?></div>
                </div>

                <div class="field-group" id="fg-duree">
                    <label for="duree">Durée (minutes)</label>
                    <input type="number" id="duree" name="duree" value="<?= htmlspecialchars($quizData['duree']) ?>">
                ...
                <div class="field-group" id="fg-seuilReussite">
                    ...
                    <input type="number" id="seuilReussite" name="seuilReussite" value="<?= htmlspecialchars($quizData['seuilReussite']) ?>">
                    <div class="field-msg" id="seuil-msg"><?= htmlspecialchars($errors['seuilReussite'] ?? 'Minimum 40%, maximum 100%') ?></div>
                </div>
                ...
                <div class="field-group" id="fg-niveau">
                    ...
                    <select id="niveau" name="niveau">
                        ...
                        <option value="Débutant"<?= $normalizeSelectValue($quizData['niveau'] ?? '') === 'debutant' ? ' selected' : '' ?>>Débutant</option>
                        <option value="Intermédiaire"<?= $normalizeSelectValue($quizData['niveau'] ?? '') === 'intermediaire' ? ' selected' : '' ?>>Intermédiaire</option>
                        <option value="Avancé"<?= $normalizeSelectValue($quizData['niveau'] ?? '') === 'avance' ? ' selected' : '' ?>>Avancé</option>
                    </select>
                    ...
                    <select id="idCourse" name="idCourse">
                        ...
                        <?php if ($courses): ?>
                            <?php foreach ($courses as $course): ?>
                                <option value="<?= htmlspecialchars($course['idCourse']) ?>" <?= (intval($quizData['idCourse'] ?? 0) === intval($course['idCourse'])) ? 'selected' : '' ?>>
                ...
                <div class="field-group" id="fg-statut">
                    ...
                    <select id="statut" name="statut">
                        <option value="">Sélectionnez</option>
                        <option value="Actif"<?= $normalizeSelectValue($quizData['statut'] ?? '') === 'actif' ? ' selected' : '' ?>>Actif</option>
                        <option value="Inactif"<?= $normalizeSelectValue($quizData['statut'] ?? '') === 'inactif' ? ' selected' : '' ?>>Inactif</option>
                        <option value="Brouillon"<?= $normalizeSelectValue($quizData['statut'] ?? '') === 'brouillon' ? ' selected' : '' ?>>Brouillon</option>
                    </select>
