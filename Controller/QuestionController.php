...
class QuestionController {
    ...
    public function validateQuestion($data) {
        // Comme pour les quiz, on centralise la validation ici avant toute écriture.
        Validator::reset();
        ...
        // Chaque champ suit la règle métier attendue par le formulaire.
        ...
        if ($type === 'QCU') {
            // Une QCU a besoin d'au moins deux propositions et d'une bonne réponse présente dans la liste.
        ...
        // Si la validation a accumulé une erreur, on refuse la suite.
    ...
    public function addQuestion($question) {
        // L'objet Question arrive déjà préparé par le contrôleur de formulaire.
        $db = Config::getConnexion();
        try {
            $this->ensureResponsesColumn($db);
    ...
    public function updateQuestion($question, $id) {
        // Même logique que l'ajout, mais en ciblant l'enregistrement déjà existant
        $db = Config::getConnexion();
        try {
            $this->ensureResponsesColumn($db);
            $responses = $question->getResponses();
    ...
    public function deleteQuestion($id) {
        // Suppression directe de la question sélectionnée.
