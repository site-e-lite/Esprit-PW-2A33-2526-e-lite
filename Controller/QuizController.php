...
class QuizController {
    ...
    // Anti-cheat: on garde une table de verrous pour bloquer/reouvrir les tentatives de quiz.
    ...
    // Anti-cheat: vérifie si cet utilisateur (ou sa session) est déjà verrouillé pour ce quiz.
    ...
    // Anti-cheat: crée un verrou si aucun verrou actif n'existe encore pour ce contexte.
    ...
    // Anti-cheat: déverrouillage manuel depuis le back-office.
    ...
    public function getAvailableQuestions($niveau = null, $limit = null) {
        // Génération auto: on prend des questions compatibles avec le niveau (ou sans niveau), au hasard.
        // Note: Les questions peuvent être associées à d'autres quiz et seront réassociées au nouveau quiz.
        $db = Config::getConnexion();
        $params = [];
    ...
    // Génération auto: associe les questions tirées au quiz nouvellement créé.
    public function assignQuestionsToQuiz($idQuiz, $questionIds, $niveauQuiz = null) {
        ...
        try {
            // Récupérer le niveau du quiz si non fourni
            if ($niveauQuiz === null) {
                $quizData = $this->getQuizById($idQuiz);
                $niveauQuiz = $quizData['niveau'] ?? null;
            }

            // Valider que toutes les questions correspondent au niveau du quiz
            ...
            // D'abord, détache toutes les questions du quiz en cas de mise à jour
            $placeholders = implode(',', array_fill(0, count($questionIds), '?'));
            $db->prepare("UPDATE question SET idQuiz = NULL WHERE idQuestion IN ($placeholders)")->execute($questionIds);
            
            // Ensuite, associe les questions sélectionnées au nouveau quiz
    ...
    // Génération auto: valide le formulaire, crée le quiz, puis lui attache les questions sélectionnées.
    ...
    // Anti-cheat: enregistre le résultat académique + les indicateurs de tentative (onglets, inactivité, rapidité).
    ...
    public function validateQuizQuestionMatch($niveau, $questionIds = []) {
        // Valide que toutes les questions assignées sont du même niveau que le quiz
        ...
        try {
            ...
            // Retourne true si toutes les questions correspondent au niveau
            return ($result['count_questions'] ?? 0) > 0 && 
                   ($result['count_questions'] == $result['count_matching']);
        } catch (Exception $e) {
            return true; // Permettre si erreur (colonne niveau peut ne pas exister)
        }
    }
