...
class UserController {
    ...
    private function redirect($path) { $bp = rtrim(str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME'])), "/"); if($bp==="."||$bp==="/") $bp=""; header("Location: $bp$path"); exit; }
    private function normalizedRoleName(): string {
        $role = trim((string)($_SESSION['role_nom'] ?? ''));
        $role = strtolower($role);
        $role = str_replace(['é', 'è', 'ê', 'ë', 'à', 'â', 'ä', 'î', 'ï', 'ô', 'ö', 'ù', 'û', 'ü', 'ç'], ['e', 'e', 'e', 'e', 'a', 'a', 'a', 'i', 'i', 'o', 'o', 'u', 'u', 'u', 'c'], $role);
        return preg_replace('/\s+/', ' ', $role) ?? '';
    }
    private function isStudent() {
        $role = $this->normalizedRoleName();
        return in_array($role, ['student', 'etudiant', 'eleve'], true);
    }
    private function isAdminOrFormateur() {
        if (!$this->isLoggedIn()) return false;
        $roleId = (int)($_SESSION['user_role'] ?? 0);
        $roleName = $this->normalizedRoleName();
        return in_array($roleId, [1], true) || in_array($roleName, ['admin', 'administrateur', 'formateur', 'teacher', 'instructor'], true);
    }
    ...
    public function googleCallback() {
        ...
        $_SESSION['user_id'] = $user['idUser']; $_SESSION['user_role'] = $user['idRole']; $_SESSION['role_nom'] = $user['role_nom'];
    ...
    public function resetPassword() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (strlen($password) < 6) {
                ...
                User::update($_SESSION['reset_user_id'], ['motDePasse' => $hash]);
