...
if ($id <= 0) { header('Location: '.$basePath.'/virtualclass/sessions?error=ID+invalide'); exit; }

$classes = $db->query('SELECT idClass, titre FROM virtualclass ORDER BY idClass DESC')->fetchAll();
$error = null;
$sess = $controller->getSessionById($id);
if (!$sess) { header('Location: '.$basePath.'/virtualclass/sessions?error=Séance+introuvable'); exit; }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $updated = new SessionClass($id, trim($_POST['dateSession']??''), trim($_POST['heureDebut']??''), trim($_POST['heureFin']??''), trim($_POST['statut']??''), (int)($_POST['idClass']??0));
        $controller->updateSession($updated, $id);
        header('Location: '.$basePath.'/virtualclass/sessions?success=Séance+mise+à+jour'); exit;
    } catch (Throwable $e) { $error = $e->getMessage(); }
}

$dateV  = $_POST['dateSession'] ?? $sess->getDateSession();
$debV   = $_POST['heureDebut']  ?? substr($sess->getHeureDebut(), 0, 5);
$finV   = $_POST['heureFin']    ?? substr($sess->getHeureFin(), 0, 5);
$statV  = $_POST['statut']      ?? $sess->getStatut();
$classV = $_POST['idClass']     ?? $sess->getIdClass();
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Modifier Séance #<?= $id ?></title>
...
<main class="main-content">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
        <h1 style="margin:0;font-size:1.8rem;"><i class="fas fa-edit" style="color:var(--accent);"></i> Modifier Séance <span style="color:var(--accent);">#<?= $id ?></span></h1>
                ...
                <div class="form-group full-width">
                    ...
                    <select name="idClass" id="se_idClass">
                        ...
                        <?php foreach($classes as $c): ?>
                            <option value="<?= (int)$c['idClass'] ?>" <?= (string)$classV===(string)$c['idClass']?'selected':'' ?>>#<?= (int)$c['idClass'] ?> — <?= htmlspecialchars($c['titre']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label>Date *</label>
                    <input type="date" name="dateSession" id="se_dateSession" value="<?= htmlspecialchars($dateV) ?>">
                </div>
                <div class="form-group">
                    <label>Statut *</label>
                    <select name="statut" id="se_statut">
                        ...
                        <?php foreach(['planifiee','en_cours','terminee','annulee'] as $st): ?>
                            <option value="<?= $st ?>" <?= $statV===$st?'selected':'' ?>><?= $st ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label>Heure début *</label>
                    <input type="time" name="heureDebut" id="se_heureDebut" value="<?= htmlspecialchars($debV) ?>">
                </div>
                <div class="form-group">
                    <label>Heure fin *</label>
                    <input type="time" name="heureFin" id="se_heureFin" value="<?= htmlspecialchars($finV) ?>">
                </div>
            </div>
            <button type="submit" class="btn-primary full-width" style="margin-top:1.2rem;"><i class="fas fa-save"></i> Mettre à jour</button>
        </form>
    </div>
</main>
